...
- **Q** [apply · predict-first] (calibration) Before answering, predict your score: match each to its canonical algorithm — O(1), O(log n), O(n), O(n log n), O(n²).
  **A** Predicted 3/5 ("shaky on n log n"); actually got 5/5: "O(1) hash lookup, O(log n) binary search, O(n) linear scan, O(n log n) mergesort, O(n²) nested-loop / bubble sort." → ✓ correct across all five. The under-confidence is the useful signal — knows more than he trusts; ease off the scaffolding faster. → 3.4 `understands`, 3.2 `understands`.
...
**Next:** Area 4.1 — counting operations (sequential vs nested), the gap from the S1 diagnostic. Use faded examples (Area 4 is `aware`, scaffold exists). Spaced-retrieval check due on 2.2/2.3. Also due before promoting Area 3 toward `can-teach`: a **transfer test** (analyse the complexity of a function *he* writes, not one handed to him) and an **altitude** push (don't just name the class — judge *which* algorithm to pick under a complexity budget).
